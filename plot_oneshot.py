...
Description: 
Author: caobin
Date: 2021-07-26 17:30:47
Github: https://github.com/bcao19
LastEditors: caobin
LastEditTime: 2021-07-26 21:04:49
'''
...
names['signal20'].place(x=400, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['signal2'+str(i)] = tk.Entry()
    names['signal2'+str(i)].place(x=380, y=10+25*i, width=80)



names['tree20'] = tk.Label(window,text="tree", height=1) #标签
names['tree20'].place(x=500, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['tree2'+str(i)] = tk.Entry()
    names['tree2'+str(i)].place(x=480, y=10+25*i, width=80)



names['label20'] = tk.Label(window,text="label", height=1) #标签
names['label20'].place(x=600, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['label2'+str(i)] = tk.Entry()
    names['label2'+str(i)].place(x=580, y=10+25*i, width=80)



names['signal30'] = tk.Label(window,text="signal", height=1) #标签
names['signal30'].place(x=700, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['signal3'+str(i)] = tk.Entry()
    names['signal3'+str(i)].place(x=680, y=10+25*i, width=80)



names['tree30'] = tk.Label(window,text="tree", height=1) #标签
names['tree30'].place(x=800, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['tree3'+str(i)] = tk.Entry()
    names['tree3'+str(i)].place(x=780, y=10+25*i, width=80)



names['label30'] = tk.Label(window,text="label", height=1) #标签
names['label30'].place(x=900, y=10) #指定包管理器放置组件

for i in range(1, 8):
    names['label3'+str(i)] = tk.Entry()
    names['label3'+str(i)].place(x=880, y=10+25*i, width=80)
...
def get_input():
    ...
    return shot, signals1, trees1, labels1, signals2, trees2, labels2, signals3, trees3, labels3, begin_time, end_time, low_frequency, up_frequency, 
...
def plot_data():
    ...
    [shot, signals1, trees1, labels1, signals2, trees2, labels2, signals3, trees3, labels3, begin, end, low_filter, up_filter] = get_input()
